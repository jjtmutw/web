# ==========================================
# LINE Notification Center v2.3 FINAL (MySQL Enterprise)
...
# -----------------------------
# MySQL settings (v2.3 Enterprise)
# -----------------------------
DB_BACKEND = (CFG.get("db_backend") or "mysql").lower()
MYSQL_HOST = CFG.get("mysql_host", "127.0.0.1")
MYSQL_PORT = int(CFG.get("mysql_port", 3306))
MYSQL_DB = CFG.get("mysql_db", "line_center")
MYSQL_USER = CFG.get("mysql_user", "root")
MYSQL_PASS = CFG.get("mysql_pass", "")
MYSQL_CHARSET = CFG.get("mysql_charset", "utf8mb4")
...
def fmt_ts(ts: Any) -> str:
    """Format timestamp/datetime for UI.
    - SQLite v2.3 stored unix int seconds
    - MySQL enterprise stores DATETIME
    """
    try:
        if ts is None:
            return ""
        if isinstance(ts, (datetime.datetime, datetime.date)):
            if isinstance(ts, datetime.date) and not isinstance(ts, datetime.datetime):
                return datetime.datetime(ts.year, ts.month, ts.day).strftime("%Y-%m-%d %H:%M:%S")
...
def init_db() -> None:
    ...
    cur.execute(
        ...
        CREATE TABLE IF NOT EXISTS identities (
            ...
            id_type ENUM('user','group','room') NOT NULL,
            ...
            member_token_exp DATETIME NULL,
            PRIMARY KEY (id),
            UNIQUE KEY uq_line_id (line_id),
            KEY idx_phone (phone),
...
def bind_phone(line_id: str, phone: str, note: str = "") -> None:
    ...
    cur = conn.cursor()
    cur.execute(
        "UPDATE identities SET phone=%s, note=%s, last_seen=%s WHERE line_id=%s",
        (phone, note, now, line_id),
    )
    conn.close()


def list_identities(q: str = "", limit: int = 500) -> List[dict]:
    conn = db()
    cur = conn.cursor()
    if q:
        like = f"%{q}%"
        cur.execute(
            ...
            WHERE line_id LIKE %s OR phone LIKE %s OR note LIKE %s
...
def get_identity_by_line_id(line_id: str) -> Optional[dict]:
    conn = db()
    cur = conn.cursor()
    cur.execute("SELECT * FROM identities WHERE line_id=%s LIMIT 1", (line_id,))
    row = cur.fetchone()
    conn.close()
    return row


def get_identity_by_phone(phone: str) -> Optional[dict]:
    conn = db()
    cur = conn.cursor()
    cur.execute(
...
def delete_identity(line_id: str) -> None:
    conn = db()
    cur = conn.cursor()
    cur.execute("DELETE FROM identities WHERE line_id=%s", (line_id,))
    conn.close()
...
def get_member(phone: str) -> Optional[dict]:
    conn = db()
    cur = conn.cursor()
    cur.execute("SELECT * FROM members WHERE phone=%s LIMIT 1", (phone,))
    row = cur.fetchone()
    conn.close()
    return row


def get_members_map(phones: List[str]) -> Dict[str, dict]:
    ...
    cur.execute(f"SELECT * FROM members WHERE phone IN ({placeholders})", phones)
    rows = cur.fetchall()
    conn.close()
...
def line_login_page():
    ...
    <body>
      ...
      <div class="hint" style="margin-top:14px">
        管理者後台：<a href="{SITE_BASE_URL}/admin" target="_blank">{SITE_BASE_URL}/admin</a>
      </div>
    </body>
    </html>
    """
        ...
        <body>
          ...
          <div class="card">
            ...
            <form method="post" action="/member/save">
              ...
              <label>群組</label>
              <div class="hint">請選擇你目前的課程/群組（供後續 API 依群組推播使用）</div>
              <select name="group_name">
                <option value="" {('selected' if not group_val else '')}>請選擇</option>
                {f'<option value="{group_val}" selected>（已存）{group_val}</option>' if (group_val and group_val not in ["114下創客產品實作","114下物聯網概論","114下大數據實作","其他"]) else ''}
                <option value="114下創客產品實作" {('selected' if group_val=="114下創客產品實作" else '')}>114下創客產品實作</option>
                <option value="114下物聯網概論" {('selected' if group_val=="114下物聯網概論" else '')}>114下物聯網概論</option>
                <option value="114下大數據實作" {('selected' if group_val=="114下大數據實作" else '')}>114下大數據實作</option>
                <option value="其他" {('selected' if group_val=="其他" else '')}>其他</option>
              </select>
            ...
            <div class="hint" style="margin-top:12px">
              儲存後可用 API 推播：
              <code>/api/push?group=醫管三&msg=...&key=...</code>
            </div>
          </div>
        </body>
        </html>
...
def member_save():
    ...
    cur = conn.cursor()
    cur.execute(
        "UPDATE identities SET note=%s WHERE phone=%s AND id_type='user'",
        (note, phone),
    )
    conn.close()
...
def admin_update():
    ...
    # 若有填手機，就同步更新 members（供 group/name 模糊查詢與其它系統共用）
    if phone:
        upsert_member(phone, name, email, group_name, remark, line_id)

        # identities.note 讓清單更好讀
        note = f"{name}<{email}>"
        if group_name:
            note += f"｜群組:{group_name}"
        if remark:
        ...
        # 沒填手機就只更新 identities 的 last_seen（避免把 phone 清空造成誤判）
        conn = db()
        cur = conn.cursor()
        cur.execute(
            "UPDATE identities SET last_seen=%s WHERE line_id=%s",
            (now, line_id),
        )
        conn.close()

    return redirect(url_for("admin_home", msg="✅ 已更新"))
